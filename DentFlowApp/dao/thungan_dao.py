...
def get_phieu_dieu_tri_chua_thanh_toan(page=1, PAGE_SIZE=20):
...
def get_ds_phieu_dieu_tri_da_thanh_toan(page=1, PAGE_SIZE=20):
    query = PhieuDieuTri.query
    query = query.filter(PhieuDieuTri.trang_thai_thanh_toan.in_([TrangThaiThanhToan.DA_THANH_TOAN, TrangThaiThanhToan.HOAN_TIEN]))
    query = query.options(
        joinedload(PhieuDieuTri.ho_so_benh_nhan),
        joinedload(PhieuDieuTri.bac_si),
        joinedload(PhieuDieuTri.chi_tiet_dich_vu)
